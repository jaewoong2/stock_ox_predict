...
```mermaid
graph TD
    A[23:59 KST 예측 마감] --> B[06:00 KST EOD 데이터 수집]
    B --> C[정산 로직 실행]
    C --> D{예측 결과}
    D -->|정답| E[100포인트 지급]
    D -->|오답| F[포인트 없음]
...
1. **일일 사이클 관리**
   ...
   - [22:00 KST] 예측 마감, CLOSED 상태 전환
   - [22:00-06:00] 정산 대기 및 처리
...
#### **4.4 정산 및 보상 시스템**
```
23:59 예측 마감 → 06:00 EOD 수집 → 정산 실행 → 100포인트 지급
```
**API 엔드포인트:**
...
#### **4.6 배치 및 자동화 시스템**
```
05:30 유니버스 생성 → 06:00 정산 → 22:00 세션 마감
```
**API 엔드포인트:**
