...
class AdUnlockService:
    ...
    MAX_COOLDOWN_UNLOCKS_PER_DAY = 5  # 일일 쿨다운 제한
        ...
        try:
            ...
            if today_ad_unlocks >= self.MAX_AD_UNLOCKS_PER_DAY:
                return AdWatchCompleteResponse(
                    ...
                    slots_unlocked=0,
                    current_max_predictions=self._get_current_max_predictions(user_id, current_date)
                )
            ...
            if not unlock_record:
                raise BusinessLogicError("광고 시청 기록 생성에 실패했습니다.")
            ...
            logger.info(f"User {user_id} watched ad and unlocked {self.SLOTS_PER_UNLOCK} slots")
            ...
            logger.error(f"Failed to process ad watch completion for user {user_id}: {str(e)}")
            raise ValidationError(f"광고 시청 처리 중 오류가 발생했습니다: {str(e)}")

    def unlock_slot_by_cooldown(
        self, 
        user_id: int
    ) -> SlotIncreaseResponse:
        """
        ...
        try:
            ...
            if not unlock_record:
                raise BusinessLogicError("쿨다운 해제 기록 생성에 실패했습니다.")
            ...
            logger.info(f"User {user_id} used cooldown to unlock {self.SLOTS_PER_UNLOCK} slots")
            ...
            logger.error(f"Failed to unlock slot by cooldown for user {user_id}: {str(e)}")
            raise ValidationError(f"쿨다운 슬롯 해제 중 오류가 발생했습니다: {str(e)}")
    ...
    def get_available_slots_info(self, user_id: int) -> AvailableSlotsResponse:
        ...
        try:
            ...
            can_unlock_by_cooldown = (
                today_cooldown_unlocks < self.MAX_COOLDOWN_UNLOCKS_PER_DAY and
                self._can_use_cooldown(user_id, current_date)
            )
            
            return AvailableSlotsResponse(
                current_max_predictions=stats.max_predictions,
                predictions_made=stats.predictions_made,
                available_predictions=max(0, stats.max_predictions - stats.predictions_made),
            ...
            logger.error(f"Failed to get available slots info for user {user_id}: {str(e)}")
    ...
    def get_user_unlock_history(
        ...
        try:
            # 원시 기록 조회
            raw_records = self.ad_unlock_repo.get_user_unlock_history(user_id, limit * 10)
            ...
            for trading_day in sorted(daily_records.keys(), reverse=True)[:limit]:
                ...
                ad_unlocks = sum(
                    r.unlocked_slots for r in records 
                ...
                history_list.append(AdUnlockHistory(
            ...
            raise ValidationError(f"해제 히스토리 조회 중 오류가 발생했습니다: {str(e)}")
