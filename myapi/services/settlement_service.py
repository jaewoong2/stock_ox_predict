...
class SettlementService:
    ...
    async def validate_and_settle_day(self, trading_day: date) -> Dict[str, Any]:
        ...
        try:
            ...
            if not settlement_data:
                raise NotFoundError(f"No price data available for settlement on {trading_day}")
            ...
            return {
                ...
                "accuracy_rate": (total_correct / total_processed * 100) if total_processed else 0,
    ...
    async def _settle_predictions_for_symbol(
        ...
        for prediction in pending_predictions:
            ...
            is_correct = self._is_prediction_correct(predicted_direction, actual_movement)
        ...
        return {
            ...
            "accuracy_rate": (correct_count / processed_count * 100) if processed_count else 0,
            "price_movement": price_data.price_movement,
    ...
    async def _award_correct_prediction(
        ...
        # 임시로 로그만 출력
        print(f"Awarded points to user {user_id} for correct prediction {prediction_id}")
    ...
    async def _void_prediction(
        ...
        print(f"Voided prediction {prediction_id} for user {user_id}, reason: {void_reason}")
    ...
    async def get_settlement_summary(self, trading_day: date) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "settlement_status": "COMPLETED" if pending_predictions == 0 else "PENDING",
    ...
    def _get_symbol_wise_stats(self, trading_day: date) -> List[Dict[str, Any]]:
        """종목별 정산 통계"""
        # 이 메서드는 실제 구현에서 repository에서 집계 쿼리를 사용해야 함
        universe_symbols = self.universe_repo.get_symbols_for_day(trading_day)
        
        symbol_stats = []
        for symbol in universe_symbols:
            predictions = self.pred_repo.get_predictions_by_symbol_and_date(symbol, trading_day)
            
            total = len(predictions)
            correct = len([p for p in predictions if p.status == StatusEnum.CORRECT])
            incorrect = len([p for p in predictions if p.status == StatusEnum.INCORRECT])
            void = len([p for p in predictions if p.status == StatusEnum.VOID])
    ...
    async def manual_settle_symbol(
        self, trading_day: date, symbol: str, correct_choice: PredictionChoice,
        ...
        try:
                ...
                async with self.price_service as price_svc:
                    try:
                        ...
                        if correct_choice.value.upper() != actual_movement:
                            print(f"Warning: Manual choice {correct_choice.value} differs from calculated movement {actual_movement}")
                    except Exception as e:
                        print(f"Could not validate price data: {e}")
            
            # 수동 정산 실행
            correct_count, total_count = self.pred_repo.bulk_update_predictions_status(
                trading_day, symbol, ChoiceEnum(correct_choice.value), points_per_correct=100
            )
            ...
            return {
                ...
                "accuracy_rate": (correct_count / total_count * 100) if total_count else 0
            }
