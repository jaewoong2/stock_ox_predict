...
class UserService:
    ...
    def get_active_users(self, limit: int = 50, offset: int = 0) -> List[UserSchema]:
        """활성 사용자 목록 조회"""
        if limit > 100:  # 최대 제한
            limit = 100
    ...
    def validate_nickname_availability(
        self, nickname: str, exclude_user_id: Optional[int] = None
    ) -> bool:
        """닉네임 사용 가능 여부 확인"""
    ...
    def get_user_profile_with_points(self, user_id: int) -> UserProfileWithPoints:
        """포인트 정보를 포함한 사용자 프로필 조회"""
        profile = self.get_user_profile(user_id)
        balance = self.get_user_points_balance(user_id)

        if profile is None or balance is None:
            raise NotFoundError(f"User profile or points not found: {user_id}")

        return UserProfileWithPoints(
    ...
    def award_signup_bonus(self, user_id: int) -> bool:
        ...
        try:
            ...
            result = self.point_service.add_points(
                user_id=user_id, request=bonus_request
            )

            if result.success:
                logger.info(
                    f"✅ Awarded signup bonus to user {user_id}: {self.settings.SIGNUP_BONUS_POINTS} points"
                )
                return True
            else:
                logger.warning(
                    f"❌ Failed to award signup bonus to user {user_id}: {result.message}"
                )
                return False
    ...
    def get_user_financial_summary(self, user_id: int) -> UserFinancialSummary:
        """사용자의 재정 요약 정보"""
        if not self.check_user_exists(user_id):
            raise NotFoundError(f"User not found: {user_id}")
