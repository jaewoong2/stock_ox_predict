...
async def get_cooldown_status(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="쿨다운 상태 조회 중 오류가 발생했습니다.")
...
async def cancel_cooldown(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="쿨다운 취소 중 오류가 발생했습니다.")
...
async def handle_slot_refill_message(
    ...
    """
    try:
        logger.info(f"Processing slot refill message: timer_id={message.timer_id}, user_id={message.user_id}")
        ...
        raise HTTPException(
            status_code=500, 
            detail=f"슬롯 충전 처리 중 오류가 발생했습니다: {str(e)}"
        )
