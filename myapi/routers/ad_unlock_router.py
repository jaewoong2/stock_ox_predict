...
async def watch_ad_complete(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="광고 시청 처리 중 오류가 발생했습니다."
        )
...
async def unlock_slot_by_cooldown(
    ...
    try:
        ...
        response = ad_unlock_service.unlock_slot_by_cooldown(user_id=current_user.id)
        ...
        raise HTTPException(
            status_code=500, detail="쿨다운 슬롯 해제 중 오류가 발생했습니다."
        )
...
async def get_available_slots_info(
    ...
    try:
        ...
        response = ad_unlock_service.get_available_slots_info(user_id=current_user.id)
        ...
        raise HTTPException(
            status_code=500, detail="슬롯 정보 조회 중 오류가 발생했습니다."
        )
...
async def get_my_unlock_history(
    ...
    try:
        logger.info(
            f"User {current_user.id} retrieving unlock history (limit: {limit})"
        )
        ...
        logger.info(
            f"User {current_user.id} retrieved {len(history)} days of unlock history"
        )
        ...
        raise HTTPException(
            status_code=500, detail="해제 히스토리 조회 중 오류가 발생했습니다."
        )
...
async def get_daily_unlock_stats(
    ...
    try:
        logger.info(
            f"Admin {current_user.id} retrieving daily unlock stats for {trading_day}"
        )
        ...
        raise HTTPException(
            status_code=500, detail="일일 통계 조회 중 오류가 발생했습니다."
        )
...
async def get_user_unlock_history_admin(
    ...
    try:
        logger.info(
            f"Admin {current_user.id} retrieving unlock history for user {user_id}"
        )
