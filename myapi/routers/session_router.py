...
def get_today_session(
    ...
    try:
        ...
        if not market_status['is_trading_day']:
            ...
            response_data["market_status"]["next_trading_day"] = next_trading_day.strftime("%Y-%m-%d")
    ...
    try:
        ...
        return BaseResponse(
            ...
            data={
                ...
                "trading_day": (target_date or USMarketHours.get_current_kst_time().date()).isoformat(),
                "current_time": USMarketHours.get_current_kst_time().strftime("%Y-%m-%d %H:%M:%S")
            }
        )
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to check prediction availability: {str(e)}"
        )
