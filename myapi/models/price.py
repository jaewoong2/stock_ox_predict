...
from sqlalchemy import Column, String, Date, DateTime, Numeric, Integer, Index
from sqlalchemy.dialects.postgresql import UUID
import uuid

from myapi.models.base import TimestampMixin, Base
...
class EODPrice(Base, TimestampMixin):
    ...
    symbol = Column(String(10), nullable=False, comment="종목 심볼")
    trading_date = Column(Date, nullable=False, comment="거래일")
    
    # 가격 정보
    open_price = Column(Numeric(10, 4), nullable=False, comment="시가")
    high_price = Column(Numeric(10, 4), nullable=False, comment="고가")
    low_price = Column(Numeric(10, 4), nullable=False, comment="저가")
    close_price = Column(Numeric(10, 4), nullable=False, comment="종가")
    adjusted_close = Column(Numeric(10, 4), nullable=True, comment="수정 종가")
    previous_close = Column(Numeric(10, 4), nullable=False, comment="전일 종가")
    
    # 변동 정보
    change_amount = Column(Numeric(10, 4), nullable=False, comment="가격 변동액")
    change_percent = Column(Numeric(6, 4), nullable=False, comment="변동률 (%)")
    
    # 거래 정보
    volume = Column(Integer, nullable=False, comment="거래량")
    
    # 메타데이터
    data_source = Column(String(50), nullable=False, default="yfinance", comment="데이터 출처")
    fetched_at = Column(DateTime, nullable=False, default=datetime.utcnow, comment="데이터 수집 시각")
