...
class CooldownTimer(Base):
    ...
    trading_day: Mapped[date] = mapped_column(
        Date, nullable=False, comment="타이머가 생성된 거래일"
    )
    ...
    eventbridge_rule_arn: Mapped[Optional[str]] = mapped_column(
        String(500), nullable=True, comment="AWS EventBridge 규칙 ARN (스케줄 취소용)"
    )
    slots_to_refill: Mapped[int] = mapped_column(
        Integer, nullable=False, default=1, comment="충전할 슬롯 개수"
    )
