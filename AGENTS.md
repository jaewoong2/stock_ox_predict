...
### ⚠️ CRITICAL: Model Inheritance Rules

**BEFORE creating or modifying ANY SQLAlchemy model, CHECK if the database table has `created_at` and `updated_at` columns!**

#### Rule 1: Check Database Schema First

```bash
# ALWAYS verify table schema before writing model code
...
class YourModel(BaseModel):  # ✅ CORRECT - includes TimestampMixin
...
class YourModel(Base):  # ✅ CORRECT - no timestamps
    """
...
class TickerReference(BaseModel):  # ❌ WRONG!
    # BaseModel adds created_at/updated_at automatically
    # But DB table doesn't have these columns
    # Result: psycopg2.errors.UndefinedColumn
```
...
class TickerReference(Base):  # ✅ CORRECT
    """No timestamps in DB table, so use Base only"""
```

#### Rule 4: When Creating New Tables
...
Then use `BaseModel` in Python:
```python
class YourModel(BaseModel):  # ✅ Table has timestamps
```

#### Rule 5: Verification Checklist

Before committing any new model:
- [ ] Checked actual database table schema
- [ ] Verified if `created_at` and `updated_at` exist in DB
- [ ] Used `BaseModel` if timestamps exist, `Base` if not
- [ ] Added docstring explaining inheritance choice
...
```
psycopg2.errors.UndefinedColumn: column your_table.created_at does not exist
LINE 1: ...field AS schema_table_field, schema.table.created_at AS ...
```

**This breaks:**
- All queries involving the model
- Repository operations
- Service layer calls
- API endpoints

**Prevention:**
1. Check DB schema FIRST (not assumptions)
2. Match model inheritance to actual table structure
3. Document why you chose Base vs BaseModel
